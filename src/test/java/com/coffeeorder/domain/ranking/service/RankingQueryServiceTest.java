...
class RankingQueryServiceTest {
	...
	void getPopularMenus_includesDayMinusSix_excludesDayMinusSeven() {
		...
		/*
		 * 이 Redis는 로컬 dev용 docker-compose를 다른 테스트·수동 bootRun과 공유해서 "오늘"
		 * 버킷에 소규모 실제/테스트 카운트가 섞여 있을 수 있다. includedMenu가 실제 top-3
		 * 안에 들어야 검증되는 케이스라 그 잡음을 확실히 이기도록 점수를 크게 잡는다
		 * (제외 쪽은 날짜 자체가 union 쿼리에 안 들어가므로 점수와 무관하게 제외되지만,
		 * 대칭성을 위해 동일하게 크게 둠).
		 */
