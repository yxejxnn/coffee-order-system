...
- `ApiResponse<T>` — `{ "code": string, "message": string|null, "data": T|null }`, `@JsonInclude(NON_NULL)`로 null 필드는 응답에서 생략.
  ...
  - 실패: `ApiResponse.error(ErrorCode)` / `error(ErrorCode, message)` / `error(String code, String message)`.
...
- **Lombok**: 공통 클래스(`ApiResponse`, `ErrorCode`, `CoffeeOrderException`)는 수동 게터 대신 `@Getter` 사용. `ErrorCode`는 `@RequiredArgsConstructor`로 생성자도 생략.
