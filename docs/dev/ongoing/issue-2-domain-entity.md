...
- 엔티티 5개: `domain.member.entity.Member`·`domain.point.entity.Point`(1:1, `@JoinColumn(member_id)` unique)·`domain.point.entity.PointHistory`(enum `PointHistoryType` STRING 매핑, `@JdbcTypeCode(SqlTypes.VARCHAR)`로 MySQL 네이티브 enum 대신 varchar(10) 강제)·`domain.menu.entity.Menu`·`domain.order.entity.Orders`(`order_group_id` unique + `idx_created_menu(created_at, menu_id)`). 생성자 직접 작성(protected no-arg + 필드 생성자), `@Getter`만 Lombok.
- 리포지토리 5개: 각 도메인 패키지의 `repository` 하위에 `JpaRepository` 인터페이스만 (커스텀 쿼리 없음, 필요 이슈에서 추가). `Orders` → `OrdersRepository`(다른 4개와 `<Entity>Repository` 패턴 통일).
- `com.coffeeorder.config.DataSeeder`(`ApplicationRunner`, 도메인 밖 — member+menu를 함께 다루는 cross-domain 컴포넌트): 테이블이 비어 있을 때만 회원 3명·메뉴 5개 시드(재기동 시 중복 삽입 방지). Point/PointHistory/Orders는 시드 범위 밖(이슈 스코프가 "회원·메뉴"로 명시).
