...
## 설계 (HOW)
- `com.coffeeorder.entity` 5개: `Member`·`Point`(1:1, `@JoinColumn(member_id)` unique)·`PointHistory`(enum `PointHistoryType` STRING 매핑, `@JdbcTypeCode(SqlTypes.VARCHAR)`로 MySQL 네이티브 enum 대신 varchar(10) 강제)·`Menu`·`Orders`(`order_group_id` unique + `idx_created_menu(created_at, menu_id)`). 생성자 직접 작성(protected no-arg + 필드 생성자), `@Getter`만 Lombok.
- `com.coffeeorder.repository` 5개: 각 엔티티당 `JpaRepository` 인터페이스만 (커스텀 쿼리 없음, 필요 이슈에서 추가).
- `com.coffeeorder.config.DataSeeder`(`ApplicationRunner`): 테이블이 비어 있을 때만 회원 3명·메뉴 5개 시드(재기동 시 중복 삽입 방지). Point/PointHistory/Orders는 시드 범위 밖(이슈 스코프가 "회원·메뉴"로 명시).
