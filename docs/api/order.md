...
## 규칙 · 동시성 · 이벤트
- 처리 순서(하나의 트랜잭션): quantity 검증 → **`Idempotency-Key`가 있고 이미 처리된 키면 여기서 즉시 반환(결제 스킵)** → 회원·메뉴 검증 → `POINT` 행 **비관적 락** → 잔액 확인·차감 → `ORDERS` 저장(`idempotency_key` 포함) → `POINT_HISTORY(USE)` 기록. → [ADR-001](../adr/ADR-001-포인트-동시성제어.md)
- 멱등성 잔여 위험(최소 스코프, #37): 동시에 같은 키로 2개 요청이 동시에 "처리된 적 없음"을 관찰하면 결제까지 각자 진행하다 `orders` 저장에서 하나만 유니크 제약을 통과한다. 진 쪽은 트랜잭션 전체가 롤백되어 이중 차감은 없지만 그 요청은 실패(500)한다 — 클라이언트가 같은 키로 다시 재시도하면 그때는 정상적으로 첫 성공 건을 받는다. 요청 body가 최초 요청과 달라도 검증하지 않는다(같은 키 → 그대로 첫 결과 반환).
