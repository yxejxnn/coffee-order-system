...
PR을 올렸으면 **Claude가 먼저 스스로 코드 리뷰**를 돌려 발견사항을 고친다. 사람에게 넘기기 전에 Claude가 정리한다.

```bash
# PR diff에 대해 코드 리뷰 실행 (예: /code-review)
# 발견사항 수정 → 커밋 → push (같은 feature 브랜치 → PR 자동 갱신)
git push
```

- 리뷰 → 수정 → 재리뷰를 **발견사항이 없어질 때까지(=approve)** 반복한다.
...
- **리뷰는 Claude가 먼저, merge는 사람만.** PR을 올리면 Claude가 자체 리뷰·수정을 통과까지 돌리고, **머지 직전에 멈춰** 사람의 최종 검토를 기다린다. `gh pr merge`는 권한에서 차단.
- **merge 후 feature 브랜치는 삭제**(로컬+원격)하고 다음 이슈로.
