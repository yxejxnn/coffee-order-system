...
### 리뷰 코멘트 스레드 처리 (resolve 여부)

코멘트를 고쳤는지 여부에 따라 GitHub 리뷰 스레드 상태를 다르게 둔다 — 사람이 최종 검토(게이트 B) 때 스레드 상태만 보고 "뭐가 반영됐고 뭐가 남았는지" 바로 알 수 있어야 한다.

- **반영한 코멘트** → 커밋·push로 고친 뒤, 그 스레드를 **Resolve conversation**으로 닫는다.
- **반영하지 않기로 한 코멘트**(사용자의 기존 결정과 상충 / 사람 판단이 필요한 트레이드오프 등) → **resolve하지 않고 그대로 열어둔 채**, 왜 안 고쳤는지 그 스레드에 **답글**을 남긴다. Resolve는 "고쳤다"는 뜻이므로, 안 고친 걸 resolve하면 사람이 놓치게 된다.
