...
실제 반영 메커니즘은 Hibernate `ddl-auto: update`다(`application.yml`). Flyway 같은 별도 마이그레이션 도구는 도입하지 않았다 — 이 과제가 운영/스테이징 프로파일 분리 없는 **단일 환경**이라, 마이그레이션 도구가 주는 이점(환경 간 스키마 이력 추적·롤백)이 적용될 대상 자체가 없기 때문이다. 대신 이 문서(`docs/db/`)가 스키마 SSOT이고, `ddl-auto: update`는 그 SSOT를 로컬 DB에 반영하는 개발 편의 수단으로만 쓴다. 스키마가 바뀌면 이 문서를 먼저 고치고 엔티티를 맞춘다 — 순서를 반대로 하지 않는다. 다중 환경으로 확장되는 시점이 오면 그때 Flyway 도입을 재검토한다.
